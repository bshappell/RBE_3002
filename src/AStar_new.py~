...
def AStar(ixInit, iyInit, ixEnd, iyEnd, iwidth, iheight):
    ...
    while(!frontier.empty()): # while there are still nodes that have not been checked, continually run the algorithm
        ...
        mapInfo = getNeighbors(currentSquare, mapInfo, width, height) # re-evaluate each neighboring node
...
def getNeighbors(currentCell,mapInfo, width, height):
