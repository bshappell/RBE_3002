...
def getNextWayPoint():  
    ...
    #transforms the click pose to A star map
    AEndGoalx = int((xClickPose - xOffset) / res)
    AEndGoaly = int((yClickPose - yOffset) / res)
    #transforms the published way point to A star map
    AWayx = int((currWayx - xOffset) / res)
    AWayy = int((currWayy - yOffset) / res)
    #transforms the robot position into A star map
    ARoPosx = (xOdom - xOffset) / res
    ARoPosy = (yOdom - yOffset) / res
    #transforms the width and height of the map to A star map
    AWidth = width
    AHeight = height

    # if the current robot position in A star is within one grid of the way point set the initial position to that way point
    if ((ANextWay[0] - 1) < ARoPosx < (ANextWay[0] + 1)) and ((ANextWay[1] - 1) < ARoPosy < (ANextWay[1] + 1)):
        AInitx = ANextWay[0]
        AInity = ANextWay[1]
    #otherwise set the initial value to the location of the robot
    else:
        AInitx = int(ARoPosx)
        AInity = int(ARoPosy)
    ...
    #get a path from a star from the start to the end position
    path = AStar(AInitx, AInity, AEndGoalx, AEndGoaly, AWidth, AHeight)
    #find all the way points from the path generated
    AWayPoints = locateWayPointsLocations(path)
    #find all the directions at each way point
    ADirections = locateWayPointsDirections(path)
    lengthWay = len(AWayPoints) 
    lengthDir = len(ADirections)
    #if there is not another waypoint in the list we are at our goal
    if (lengthWay < 1 or lengthDir <1):
        print "at the goal"
        robotAtFinalGoal = True
    #if there is a way point left
    else:
        # get the next way point the robot needs to move to
        ANextWay = AWayPoints[lengthWay - 1]
        print "A*wayX" , ANextWay[0] , "A*wayY" , ANextWay[1]
        # get the next way point direction the robot needs to move to
        ANextDir = ADirections[lengthDir - 1] 
        #transform the a star way point to the global map   
        currWayx = float(ANextWay[0] * res) + xOffset
        currWayy = float(ANextWay[1] * res) + yOffset
        print "currWayX" , currWayx , "currWayy" , currWayy
        nextDir = ANextDir
        # make the next waypoint a tuple
        nextWay = (currWayx , currWayy)
        #publish the way point
...
if __name__ == '__main__':
    ...
    while 1 and not rospy.is_shutdown():
        ...
        if((xClickPose - .1) < xOdom < (xClickPose + .1)):
            if((yClickPose - .1) < yOdom < (yClickPose + .1)):
                robotAtFinalGoal = True 
        else:
        ...
        if not robotAtFinalGoal and not rospy.is_shutdown():
            if newGoalReceived:
                newGoalReceived = False
                getNextWayPoint() 
                print "getting next way 1"
